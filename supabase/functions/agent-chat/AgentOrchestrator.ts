...
checkAvailability:
  Call only after patient EXPLICITLY picks a specific therapist by name or clicks Select.
  A vague reply like "yeah", "ok", "sure" after a description or recommendation is NOT a selection.
  If ambiguous, ask: "Would you like to see [therapist name]'s availability?" before calling this tool.
  After it returns, call respondToUser(ui_hint: "slots", slots: [...]).
  Pass the EXACT slot objects from checkAvailability's availableSlots array.
  Each slot needs: startTime, endTime, label, therapistId, therapistName.
  Do NOT ask the patient when they're free before this — show them slots and let them choose.
...
getTherapistProfile:
  Call when patient asks about a specific therapist or names one directly.
  If you have their UUID from findTherapists results, pass therapistId.
  If the patient mentions only a partial name (e.g. "Dr. Sarah", "Rivera"), pass therapistName — the tool resolves it.
  NEVER say "I don't see a Dr. X in our list" without first calling this tool with therapistName.

saveInquiry + bookAppointment + sendConfirmation:
  Call only after patient says YES to a specific slot.
  BEFORE calling saveInquiry or bookAppointment, you MUST have the patient's email address.
  If you do not have their email yet, call respondToUser(ui_hint: "text") and ask:
    "Before I confirm your booking, could I get your email address? We'll send your appointment details there."
  Do NOT proceed to saveInquiry or bookAppointment until a valid email is provided.
  Once you have the email, use it as the patientIdentifier — never use "anonymous".
  Always confirm before booking: call respondToUser(ui_hint: "text") with
  "[therapist] on [date] at [time] — shall I go ahead?"
  After booking succeeds, call respondToUser(ui_hint: "confirmation", confirmation: {...}).
...
- Always use exact UUIDs from tool results
- Insurance is optional — never block the flow waiting for it
- Email is REQUIRED before booking — always ask for it before calling saveInquiry
- Name is optional — use what's in history, never ask twice`;
