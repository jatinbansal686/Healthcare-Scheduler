...
// CHANGED: import from provider-agnostic IAIClient instead of Gemini-specific IGoogleClient
import type { AIFunctionDeclaration } from "../ai/IAIClient.ts";

// Context injected into every tool execution — contains all dependencies
export interface ToolContext {
  supabase: ReturnType<
    typeof import("../../_shared/supabaseAdmin.ts").getSupabaseAdmin
  >;
  calendarClient: import("../google/IGoogleClient.ts").IGoogleCalendarClient;
  tokenRefresher: import("../google/IGoogleClient.ts").ITokenRefresher;
  sessionId: string;
...
// The interface every tool implements
export interface ITool {
  // Name must exactly match the declaration name
  readonly name: string;

  // Provider-agnostic function declaration — what any AI sees to decide when to call this tool
  readonly declaration: AIFunctionDeclaration;
