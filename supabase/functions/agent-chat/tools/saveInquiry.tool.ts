...
export class SaveInquiryTool implements ITool {
  ...
  async execute(
    ...
    try {
      ...
      const insertPayload = {
        patient_identifier: (args.patientIdentifier as string) || "anonymous",
        ...
        // requested_schedule mirrors extracted_schedule for admin readability
        requested_schedule: (args.extractedSchedule as string) || null,
        // insurance_info mirrors extracted_insurance for admin readability
        insurance_info: (args.extractedInsurance as string) || null,
        // primary_specialty: first condition as the leading concern
        primary_specialty:
          Array.isArray(args.extractedConditions) &&
          args.extractedConditions.length > 0
            ? (args.extractedConditions as string[])[0]
            : null,
        // raw_chat_summary: store patient name here since there's no name column
        raw_chat_summary: args.patientName
          ? `Patient name: ${args.patientName as string}`
          : null,
        status: "pending",
      };
