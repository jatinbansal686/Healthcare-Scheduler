...
// Added: Google Calendar connect/disconnect banner (SOLID: OCP — new section added, nothing modified)
// ============================================================
...
import type {
  ...
  DashboardData,
  DashboardStats,
} from "../services/ITherapistService";
...
const AppointmentCard: React.FC<AppointmentCardProps> = ({
  ...
  return (
    ...
    >
      ...
      {/* Rejection reason */}
      {appt.status === "cancelled_by_therapist" &&
        appt.therapist_rejection_reason && (
          <div className="border-t border-slate-100 pt-2 mt-2">
            <p className="text-xs text-slate-400">
              Reason: {appt.therapist_rejection_reason}
            </p>
          </div>
        )}
    </div>
...
// New standalone component — no existing code modified (OCP)
...
const CalendarBanner: React.FC<CalendarBannerProps> = ({
  isConnected,
  calendarId,
  therapistId,
  session,
  ...
  const handleConnect = () => {
    try {
      const url = therapistService.buildGoogleOAuthUrl(therapistId);
      window.location.href = url;
    } catch (err) {
      logger.error(CONTEXT, "Failed to build OAuth URL", err);
      alert(
  ...
  const handleDisconnect = async () => {
    if (
      !confirm(
        "Disconnect Google Calendar? Availability checks will use simulated slots until you reconnect.",
      )
    )
      return;
    setDisconnecting(true);
    try {
      await therapistService.disconnectGoogleCalendar(session, therapistId);
      onDisconnect();
    } catch (err) {
      logger.error(CONTEXT, "Disconnect failed", err);
        ...
        <div className="flex items-center gap-2.5">
          ...
          <svg className="w-5 h-5 shrink-0" viewBox="0 0 24 24" fill="none">
            <rect
              ...
              strokeWidth="1.5"
            />
            <path d="M3 9h18" stroke="#34A853" strokeWidth="1.5" />
            <path
              d="M8 2v4M16 2v4"
              stroke="#34A853"
              strokeWidth="1.5"
              strokeLinecap="round"
            />
            <circle cx="12" cy="15" r="2.5" fill="#34A853" />
          </svg>
          <div>
            ...
            {calendarId && (
              <p className="text-xs text-emerald-600 font-mono">{calendarId}</p>
            )}
          </div>
        </div>
        <button
          ...
          className="text-xs text-slate-500 hover:text-red-600 transition-colors px-2 py-1 rounded-lg hover:bg-red-50 shrink-0 disabled:opacity-50"
        >
  ...
  return (
    <div className="mb-6 rounded-xl border border-blue-200 bg-blue-50 px-4 py-4 flex items-start justify-between gap-4">
      <div className="flex items-start gap-3">
        ...
        <div>
          ...
          <p className="text-xs text-blue-600 mt-0.5">
            Enable real-time availability checks and automatic appointment
            creation on your calendar.
          </p>
        </div>
      </div>
      <button
...
const TherapistDashboardPage: React.FC = () => {
  ...
  // Calendar connected state — derived from dashData.therapist but also toggled locally on disconnect
  ...
  // Show success toast if arriving from OAuth callback
  useEffect(() => {
    if (searchParams.get("calendar") === "connected") {
      ...
      // Clean the query param from URL without a re-render
      window.history.replaceState({}, "", "/therapist/dashboard");
    }
  }, [searchParams]);
  ...
  const loadData = useCallback(async () => {
    ...
    setDataError(null);
    try {
      const data = await therapistService.getDashboardData(session, "all");
      setDashData(data);
      // Sync calendar state from therapist profile
      setCalendarConnected(
        data.therapist.calendar_connected ??
          !!data.therapist.google_calendar_id,
      );
      setCalendarId(data.therapist.google_calendar_id ?? null);
    } catch (err) {
  ...
  // Called by CalendarBanner after successful disconnect
  ...
  const filteredAppointments =
    dashData?.appointments.filter((a) => {
        ...
        {(stats?.pending ?? 0) > 0 && (
          <div className="mb-6 rounded-xl bg-amber-50 border border-amber-200 px-4 py-3 flex items-center gap-3">
            ...
            <p className="text-sm text-amber-700">
              You have <strong>{stats?.pending}</strong> pending appointment
              {stats?.pending !== 1 ? "s" : ""} awaiting your confirmation.
            </p>
          </div>
        )}
