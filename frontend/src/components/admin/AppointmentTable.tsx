...
// ============================================================
// AppointmentTable.tsx — Admin table for booked appointments
// FIX: Patient name now uses patient_identifier directly as primary
//      source, falls back to admin_notes regex, then "—".
//      admin_notes regex kept as fallback — nothing removed.
// ============================================================
...
/**
 * Patient name resolution priority:
 * 1. patient_identifier (set directly on the appointment row)
 * 2. admin_notes regex match "Patient: <name>"
 * 3. "—"
 */
function resolvePatientName(row: AppointmentRow): string {
  if (row.patient_identifier && row.patient_identifier.trim()) {
    return row.patient_identifier.trim();
  }
  if (row.admin_notes) {
    const match = row.admin_notes.match(/^Patient:\s*(.+)$/m);
...
export const AppointmentTable: React.FC<AppointmentTableProps> = ({
  ...
  return (
    <>
      ...
      <div className="md:hidden flex flex-col gap-3">
        {appointments.map((row) => {
          const patient = resolvePatientName(row);
          const therapist = getTherapistName(row);
          const statusCls =
            statusColors[row.status] ?? "bg-slate-100 text-slate-500";
          ...
          <tbody className="divide-y divide-slate-50">
            {appointments.map((row) => (
              <tr key={row.id} className="hover:bg-slate-50 transition-colors">
                ...
                <td className="px-4 py-3">
                  <span
                    className={`inline-block rounded-full px-2 py-0.5 text-xs font-medium capitalize ${statusColors[row.status] ?? "bg-slate-100 text-slate-500"}`}
                  >
                    {row.status}
                  </span>
                </td>
