...
export const AppointmentTable: React.FC<AppointmentTableProps> = ({
  ...
  return (
    <div className="overflow-x-auto rounded-xl border border-slate-200 bg-white shadow-sm -mx-0">
      <table className="min-w-full divide-y divide-slate-100">
        <thead className="bg-slate-50">
          <tr>
            {[
        ...
        <tbody className="divide-y divide-slate-50">
          {appointments.map((row) => (
            <tr key={row.id} className="hover:bg-slate-50 transition-colors">
              ...
              <td className="px-3 sm:px-4 py-2.5 sm:py-3 text-xs text-slate-400 font-mono">
                {row.google_calendar_event_id ? (
                  <span className="text-teal-600 truncate max-w-[6rem] sm:max-w-[8rem] block">
                    {row.google_calendar_event_id.slice(0, 12)}…
                  </span>
                ) : (
                  "—"
                )}
