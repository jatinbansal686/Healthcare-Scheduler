...
// Renders clickable appointment slot cards inside a chat bubble.
// When patient clicks a card, it fires sendMessage() with the
// slot label — the agent receives it as a normal user message
// and proceeds with booking. Zero backend changes needed.
// ============================================================
...
function parseDisplayParts(label: string): {
  ...
  // Try to extract: "Tuesday, Apr 28 at 10:00 AM"
        ...
        {slots.map((slot, i) => {
          ...
          return (
            <button
              ...
              disabled={disabled}
              className="
                group flex items-center gap-3 rounded-xl border border-teal-200
                bg-teal-50 px-4 py-3 text-left transition-all duration-150
            ...
            >
              {/* Calendar icon */}
              <div className="flex-shrink-0 h-9 w-9 rounded-lg bg-teal-600 flex items-center justify-center text-white">
                <svg
              ...
              <div className="flex-1 min-w-0">
                ...
                {time && (
                  <p className="text-sm text-teal-600 font-semibold">{time}</p>
                )}
                {!day && !date && (
