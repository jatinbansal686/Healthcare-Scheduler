...
//         Legacy regex path kept 100% intact as fallback.
//         No existing props, types, or rendering logic removed.
// UI CHANGE: Fully responsive — mobile-first, no horizontal overflow.
// ============================================================
...
// ── Inline therapist card ─────────────────────────────────────
// Accepts both TherapistOption (structured) and ParsedTherapist (legacy).

interface TherapistCardInlineProps {
  therapist: TherapistOption | ParsedTherapist;
  onSelect: (name: string) => void;
  disabled: boolean;
}

function TherapistCardInline({
  therapist,
  onSelect,
  disabled,
}: TherapistCardInlineProps) {
  return (
    <div className="flex items-start sm:items-center justify-between gap-3 p-2.5 sm:p-3 rounded-lg border border-slate-200 bg-slate-50 hover:bg-teal-50 hover:border-teal-300 transition-colors">
      <div className="flex flex-col gap-0.5 flex-1 min-w-0">
        {/* Name — truncate only on very long names */}
      ...
      <button
        onClick={() => onSelect(therapist.name)}
        disabled={disabled}
        className="flex-shrink-0 px-3 py-1.5 text-xs font-medium bg-teal-600 text-white rounded-md hover:bg-teal-700 active:bg-teal-800 disabled:opacity-40 transition-colors focus:outline-none focus:ring-2 focus:ring-teal-400 focus:ring-offset-1"
      >
        Select
      </button>
    </div>
      ...
      <div className="text-sm text-slate-700 space-y-1">
        ...
        {meetLink && (
          <a
            ...
            target="_blank"
            rel="noopener noreferrer"
            className="inline-block mt-1.5 text-xs text-teal-600 underline underline-offset-2 hover:text-teal-800 break-all"
          >
            Join Google Meet →
          </a>
        )}
...
export const MessageBubble: React.FC<MessageBubbleProps> = ({
  ...
  // Priority 1: structuredData from respondToUser tool
  const hasStructuredTherapists =
    !isUser && sd?.ui_hint === "therapists" && (sd.therapists?.length ?? 0) > 0;
  const hasStructuredSlots =
    !isUser && sd?.ui_hint === "slots" && (sd.slots?.length ?? 0) > 0;
  ...
  // Priority 2/3: legacy fallback (regex) — only when no structuredData
  const legacySlots = !isUser && !sd ? (message.slots ?? []) : [];
  const hasLegacySlots = legacySlots.length > 0;
  const legacyTherapists: ParsedTherapist[] =
    !isUser && !sd ? parseTherapists(message.text) : [];
  ...
  return (
    <div
      className={`flex w-full ${isUser ? "justify-end" : "justify-start"} mb-2 sm:mb-3`}
    >
      {/* AI avatar */}
      {!isUser && (
        <div className="mr-1.5 sm:mr-2 mt-0.5 flex-shrink-0 h-7 w-7 sm:h-8 sm:w-8 rounded-full bg-teal-100 flex items-center justify-center text-teal-700 text-xs sm:text-sm font-bold self-start">
          AI
        </div>
      )}
      ...
      <div
        className={`
          ...
          ${
            hasExtras
              ? "max-w-[92%] sm:max-w-[88%] md:max-w-[82%]"
              : "max-w-[80%] sm:max-w-[72%] md:max-w-[65%]"
          }
          min-w-0
        `}
      >
      ...
      {/* User avatar */}
      {isUser && (
        <div className="ml-1.5 sm:ml-2 mt-0.5 flex-shrink-0 h-7 w-7 sm:h-8 sm:w-8 rounded-full bg-slate-200 flex items-center justify-center text-slate-600 text-xs sm:text-sm font-bold self-start">
          You
        </div>
      )}
    </div>
